...
(define (quad->string q)
  (let loop : String ([x : USQ q])
    (cond
      [(string? x) x]
      [(quad? x) (string-append* (for/list : (Listof String)
                                   ([q : USQ (quad-list x)]) (loop q)))]
  ...
  (if (eq? '() qs)
      ...
      (let loop
        ([qs : (Listof Quad) qs]
         ...
         [candidate-attr-pairs : (Listof (Pairof Symbol Any))
           (let ([first-attrs (make-quadattrs (quad-attrs (car qs)))])
             (if first-attrs
               (for/fold ([caps : QuadAttrs '()])
                         ([cap : (Pairof Symbol Any) (in-list first-attrs)])
        ...
        (cond
          ...
          [else (loop (cdr qs) (filter (λ([cap : (Pairof Symbol Any)]) (member cap (quad-attrs (car qs)))) candidate-attr-pairs))]))))
